...
const patchCategoryList = async (req,res,next) =>{
    try {
        ...
        const resFindCategoryList = await categories_list.findOne({where :{category_lists_id}})
        const extName = newCategory.categoryImage.split('.');

        const resUpdateCategoryList = await resFindCategoryList.update({
          category: newCategory.category,
          categoryImage: `http://localhost:8000/public/categoriesImage/${category_lists_id}.${extName[1]}`,
        });
